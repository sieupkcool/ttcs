...
def homePage(request):
    ...
    if request.method=='POST':
        ...
        description = ''.join(caption[:40])
        description+='...'

        notification = Notification.objects.create(user_id=post.user_id,user_create=request.user,type='created a post',link='/comment/'+post.post_id,description=description)
        notification.save()
        return redirect('homePage')
